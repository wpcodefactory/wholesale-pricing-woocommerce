...
class Alg_WC_Wholesale_Pricing_Settings_All_Products extends Alg_WC_Wholesale_Pricing_Settings_Section {
	 ...
	 * @todo    (dev) add link to "User Roles" section
	 * @todo    (dev) `alg_wc_wholesale_pricing_all_products_enabled`: better description
	 * @todo    (dev) "price directly"
	 */
	function get_settings() {
		...
		$levels_settings = array(
			array(
				...
				'desc'  => sprintf(
					...
					__( 'You can exclude selected products in "%s" section below.', 'wholesale-pricing-woocommerce' ),
		...
		foreach ( array( 'include', 'exclude' ) as $include_or_exclude ) {
			...
			foreach ( $current_products as $product_id ) {
				...
				$products[ $include_or_exclude ][ esc_attr( $product_id ) ] = (
					is_object( $product ) ?
					esc_html( wp_strip_all_tags( $product->get_formatted_name() ) ) :
					sprintf(
						/* Translators: %d: Product ID. */
						esc_html__( 'Product #%d', 'wholesale-pricing-woocommerce' ),
						$product_id
					)
				);
			}
